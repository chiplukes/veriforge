cdef inline void _whole_assign_signal(SimCtx *c, int dst_sid, int src_sid) noexcept nogil:
    ...
    if dst_words > 0:
        ...
        if src_words > 0:
            ...
            new_v = c.val[src_sid] & wmask(c.width[dst_sid])
...
cdef inline void _whole_stage_and_signal(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid) noexcept nogil:
    ...
    if dst_words > 0:
        for i in range(dst_words):
            ...
            remaining_w = c.width[dst_sid] - (i * 64)
            tail_mask = _word_mask64(remaining_w)
            c.wide_nba_val[c.wide_offset[dst_sid] + i] = out_v & tail_mask
            c.wide_nba_mask[c.wide_offset[dst_sid] + i] = out_m & tail_mask
...
cdef inline void _whole_stage_add_signal_shl(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift) noexcept nogil:
    ...
    cdef int i, src_index, remaining_w, curr_index = -1
...
cdef inline void _whole_stage_add_signal_shr(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift) noexcept nogil:
    ...
    cdef int src_words = (src_width + 63) >> 6
    cdef int out_words = dst_words if dst_words > 0 else 1
    cdef int i, src_index, remaining_w, curr_index = -1, next_index = 0
    cdef int word_shift = shift >> 6
    cdef int bit_shift = shift & 63
...
cdef inline void _umul64_addcarry(unsigned long long a, unsigned long long b, unsigned long long carry_in, unsigned long long *out_lo, unsigned long long *out_carry) noexcept nogil:
    ...
    cdef unsigned long long carry_flag = 0
    cdef unsigned long long a0 = a & mask32
    cdef unsigned long long a1 = a >> 32
    cdef unsigned long long b0 = b & mask32
    cdef unsigned long long b1 = b >> 32
    cdef unsigned long long p0 = a0 * b0
    cdef unsigned long long p1 = a0 * b1
    cdef unsigned long long p2 = a1 * b0
    cdef unsigned long long p3 = a1 * b1
    cdef unsigned long long mid = (p0 >> 32) + (p1 & mask32) + (p2 & mask32)
    cdef unsigned long long lo = (p0 & mask32) | ((mid & mask32) << 32)
    cdef unsigned long long hi = p3 + (p1 >> 32) + (p2 >> 32) + (mid >> 32)
    cdef unsigned long long lo2 = lo + carry_in
    if lo2 < lo:
        carry_flag = 1
    cdef unsigned long long carry2 = hi + carry_flag
    out_lo[0] = lo2
    out_carry[0] = carry2

cdef inline bint _signal_has_x(SimCtx *c, int sid) noexcept nogil:
    cdef int src_words = c.wide_words[sid] if c.wide_words[sid] > 0 else 1
    cdef int i
    cdef unsigned long long word_m, tail_mask
    for i in range(src_words):
        tail_mask = _word_mask64(c.width[sid] - (i * 64))
        word_m = _sig_word_mask(c, sid, i) & tail_mask
        if word_m != 0:
            return True
    return False

cdef inline unsigned long long _sig_u16(SimCtx *c, int sid, int digit_index) noexcept nogil:
    cdef int bit_index = digit_index << 4
    cdef int remaining = c.width[sid] - bit_index
    cdef int word_index = bit_index >> 6
    cdef int bit_offset = bit_index & 63
    cdef int src_words = c.wide_words[sid] if c.wide_words[sid] > 0 else 1
    cdef unsigned long long word_v
    if remaining <= 0:
        return 0
    word_v = _sig_word_val(c, sid, word_index) >> bit_offset
    if bit_offset > 48 and word_index + 1 < src_words:
        word_v |= _sig_word_val(c, sid, word_index + 1) << (64 - bit_offset)
    if remaining < 16:
        return word_v & _word_mask64(remaining)
    return word_v & (<unsigned long long>0xFFFF)

cdef inline unsigned long long _mul_signal_next_word(SimCtx *c, int lhs_sid, int rhs_sid, int lhs_digits, int rhs_digits, int prod_digits, int *digit_index, unsigned long long *carry) noexcept nogil:
    cdef int part, start_i, end_i, j, d = digit_index[0]
    cdef unsigned long long word = 0
    cdef unsigned long long acc, digit
    if d >= prod_digits:
        return 0
    for part in range(4):
        if d >= prod_digits:
        ...
        for j in range(start_i, end_i + 1):
            acc += _sig_u16(c, lhs_sid, j) * _sig_u16(c, rhs_sid, d - j)
        digit = acc & (<unsigned long long>0xFFFF)
        carry[0] = acc >> 16
        word |= digit << (part * 16)
        d += 1
    digit_index[0] = d
...
cdef inline void _whole_stage_mul_signal_shl(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift) noexcept nogil:
    cdef int dst_words = c.wide_words[dst_sid]
    cdef int out_words = dst_words if dst_words > 0 else 1
    ...
    cdef int i, src_index, remaining_w, curr_index = -1, digit_index = 0
    cdef int word_shift = shift >> 6
    cdef int bit_shift = shift & 63
...
cdef inline void _whole_stage_mul_signal_shr(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift) noexcept nogil:
    ...
    cdef int prod_words = (prod_width + 63) >> 6
    cdef int prod_digits = (prod_width + 15) >> 4
    cdef int i, src_index, remaining_w, curr_index = -1, next_index = 0, digit_index = 0
    cdef int word_shift = shift >> 6
    cdef int bit_shift = shift & 63
...
cdef inline object _sig_py_unsigned(SimCtx *c, int sid):
    ...
    cdef int i
    cdef object value = 0
    for i in range(src_words - 1, -1, -1):
        value = (value << 64) | _sig_word_val(c, sid, i)
    return value & ((((<object>1) << c.width[sid])) - 1)

cdef inline object _sig_py_mask(SimCtx *c, int sid):
    cdef int src_words = c.wide_words[sid] if c.wide_words[sid] > 0 else 1
    cdef int i
    cdef object value = 0
    for i in range(src_words - 1, -1, -1):
        value = (value << 64) | _sig_word_mask(c, sid, i)
    return value & ((((<object>1) << c.width[sid])) - 1)

cdef inline void _whole_stage_sar_sub_const_xor_signal(SimCtx *c, int dst_sid, int sub_sid, unsigned long long sub_const, int sub_const_width, int xor_sid, int shift):
    ...
    if fill_mask != 0:
        if sign_is_unknown:
            ...
            xor_val |= fill_mask
    _whole_stage_py_bits(c, dst_sid, xor_val, xor_mask)

cdef inline void _whole_stage_sar_const_sub_xor_signal(SimCtx *c, int dst_sid, int sub_sid, unsigned long long sub_const, int sub_const_width, int xor_sid, int shift):
    cdef int sub_width = c.width[sub_sid] if c.width[sub_sid] >= sub_const_width else sub_const_width
    cdef int result_width = sub_width if sub_width >= c.width[xor_sid] else c.width[xor_sid]
    cdef int fill_start = result_width - shift
    cdef object sub_mask, width_mask, dst_mask, sub_val, xor_raw_val, xor_raw_mask, diff_val, xor_val, xor_mask, sign_bit, sign_is_one, sign_is_unknown, fill_mask
...
cdef inline void _whole_stage_sar_mask_xor_signal(SimCtx *c, int dst_sid, int mask_sid, unsigned long long mask_const, int mask_const_width, int xor_sid, int shift):
...
cdef inline void _whole_stage_sar_or_const_xor_signal(SimCtx *c, int dst_sid, int or_sid, unsigned long long or_const, int or_const_width, int xor_sid, int shift):
...
cdef inline void _whole_stage_sar_xor_const_xor_signal(SimCtx *c, int dst_sid, int xor1_sid, unsigned long long xor_const, int xor_const_width, int xor_sid, int shift):
...
cdef inline void _whole_stage_sar_add_const_xor_signal(SimCtx *c, int dst_sid, int add_sid, unsigned long long add_const, int add_const_width, int xor_sid, int shift):
...
cdef inline void _whole_stage_sar_xor_xor_signal(SimCtx *c, int dst_sid, int xor1_sid, int xor2_sid, int xor_sid, int shift):
...
cdef inline void _whole_stage_sar_or_xor_signal(SimCtx *c, int dst_sid, int or1_sid, int or2_sid, int xor_sid, int shift):
...
cdef inline void _whole_stage_sar_and_xor_signal(SimCtx *c, int dst_sid, int and1_sid, int and2_sid, int xor_sid, int shift):
...
cdef inline void _whole_stage_sar_add_xor_signal(SimCtx *c, int dst_sid, int add1_sid, int add2_sid, int xor_sid, int shift):
...
cdef inline void _whole_stage_sar_sub_xor_signal(SimCtx *c, int dst_sid, int sub1_sid, int sub2_sid, int xor_sid, int shift):
...
cdef inline void _whole_stage_sar_add_signal(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift):
    ...
    _whole_stage_py_value(c, dst_sid, sum_val >> shift)

cdef inline void _whole_stage_sar_sub_signal(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift):
    cdef int result_width = c.width[lhs_sid] if c.width[lhs_sid] >= c.width[rhs_sid] else c.width[rhs_sid]
    ...
    _whole_stage_py_value(c, dst_sid, diff_val >> shift)

cdef inline void _whole_stage_sar_and_signal(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift):
    cdef int result_width = c.width[lhs_sid] if c.width[lhs_sid] >= c.width[rhs_sid] else c.width[rhs_sid]
    cdef int fill_start = result_width - shift
    ...
    if fill_mask != 0:
        if sign_is_unknown:
            ...
            and_val |= fill_mask
    _whole_stage_py_bits(c, dst_sid, and_val, and_mask)

cdef inline void _whole_stage_sar_or_signal(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift):
    cdef int result_width = c.width[lhs_sid] if c.width[lhs_sid] >= c.width[rhs_sid] else c.width[rhs_sid]
    cdef int fill_start = result_width - shift
    ...
    if fill_mask != 0:
        if sign_is_unknown:
            ...
            or_val |= fill_mask
    _whole_stage_py_bits(c, dst_sid, or_val, or_mask)

cdef inline void _whole_stage_sar_xor_signal(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift):
    cdef int result_width = c.width[lhs_sid] if c.width[lhs_sid] >= c.width[rhs_sid] else c.width[rhs_sid]
    cdef int fill_start = result_width - shift
...
cdef inline void _whole_stage_all_x(SimCtx *c, int dst_sid):
    cdef int dst_words = c.wide_words[dst_sid]
    cdef int out_words = dst_words if dst_words > 0 else 1
    cdef int i, remaining_w
    ...
    for i in range(out_words):
        ...
        tail_mask = _word_mask64(remaining_w)
        if dst_words > 0:
            c.wide_nba_val[c.wide_offset[dst_sid] + i] = 0
            c.wide_nba_mask[c.wide_offset[dst_sid] + i] = tail_mask
        else:
            c.nba_val[dst_sid] = 0
            c.nba_mask[dst_sid] = <long long>tail_mask
    if dst_words > 0:
        c.nba_val[dst_sid] = 0
        c.nba_mask[dst_sid] = <long long>c.wide_nba_mask[c.wide_offset[dst_sid]]
    c.nba_dirty[dst_sid] = 1
...
cdef inline void _whole_stage_py_value(SimCtx *c, int dst_sid, object value):
    ...
    for i in range(out_words):
        ...
        if dst_words > 0:
            c.wide_nba_val[c.wide_offset[dst_sid] + i] = out_v
            c.wide_nba_mask[c.wide_offset[dst_sid] + i] = 0
        else:
            c.nba_val[dst_sid] = <long long>out_v
...
cdef inline void _whole_stage_py_bits(SimCtx *c, int dst_sid, object value, object mask):
    ...
    for i in range(out_words):
        ...
        if dst_words > 0:
            c.wide_nba_val[c.wide_offset[dst_sid] + i] = out_v
            c.wide_nba_mask[c.wide_offset[dst_sid] + i] = out_m
        else:
            c.nba_val[dst_sid] = <long long>out_v
            c.nba_mask[dst_sid] = <long long>out_m
    if dst_words > 0:
        c.nba_val[dst_sid] = <long long>c.wide_nba_val[c.wide_offset[dst_sid]]
        c.nba_mask[dst_sid] = <long long>c.wide_nba_mask[c.wide_offset[dst_sid]]
    c.nba_dirty[dst_sid] = 1
...
cdef inline void _whole_stage_div_signal_shl(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift):
    ...
    if _signal_has_x(c, lhs_sid) or _signal_has_x(c, rhs_sid):
        _whole_stage_x_signal_shl(c, dst_sid, result_sid, shift)
        return
    lhs_val = _sig_py_unsigned(c, lhs_sid)
    rhs_val = _sig_py_unsigned(c, rhs_sid)
    if rhs_val == 0:
    ...
    _whole_stage_py_value(c, dst_sid, shifted)

cdef inline void _whole_stage_div_signal_shr(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift):
    cdef int result_sid = lhs_sid if c.width[lhs_sid] >= c.width[rhs_sid] else rhs_sid
    cdef object lhs_val, rhs_val, quotient
    if _signal_has_x(c, lhs_sid) or _signal_has_x(c, rhs_sid):
        _whole_stage_x_signal_shr(c, dst_sid, result_sid, shift)
        return
    lhs_val = _sig_py_unsigned(c, lhs_sid)
    rhs_val = _sig_py_unsigned(c, rhs_sid)
    if rhs_val == 0:
        ...
        return
    quotient = lhs_val // rhs_val
    _whole_stage_py_value(c, dst_sid, quotient >> shift)

cdef inline void _whole_stage_mod_signal_shl(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift):
    cdef int result_sid = lhs_sid if c.width[lhs_sid] >= c.width[rhs_sid] else rhs_sid
    cdef int result_width = c.width[lhs_sid] if c.width[lhs_sid] >= c.width[rhs_sid] else c.width[rhs_sid]
...
cdef inline void _whole_stage_mod_signal_shr(SimCtx *c, int dst_sid, int lhs_sid, int rhs_sid, int shift):
    cdef int result_sid = lhs_sid if c.width[lhs_sid] >= c.width[rhs_sid] else rhs_sid
    ...
    if rhs_val == 0:
        ...
        return
    remainder_val = lhs_val % rhs_val
    _whole_stage_py_value(c, dst_sid, remainder_val >> shift)

cdef inline void _whole_stage_div_const_shl(SimCtx *c, int dst_sid, int div_sid, unsigned long long rhs_const, int shift) noexcept nogil:
    cdef int dst_words = c.wide_words[dst_sid]
    cdef int src_words = c.wide_words[div_sid] if c.wide_words[div_sid] > 0 else 1
    cdef int dst_offset = c.wide_offset[dst_sid]
    cdef int i, out_index, src_index, digit_index, word_index, digit_bits, remaining_w
    cdef int word_shift = shift >> 6
    cdef int bit_shift = shift & 63
    ...
    for i in range(src_words):
        ...
        if word_m != 0:
            _whole_stage_x_signal_shl(c, dst_sid, div_sid, shift)
            return
    for i in range(dst_words):
    ...
    for i in range(dst_words):
        ...
        tail_mask = _word_mask64(remaining_w)
        c.wide_nba_val[dst_offset + out_index] = out_v & tail_mask
...
cdef inline void _whole_stage_div_const_shr(SimCtx *c, int dst_sid, int div_sid, unsigned long long rhs_const, int shift) noexcept nogil:
    ...
    for i in range(src_words):
        word_m = _sig_word_mask(c, div_sid, i) & _word_mask64(c.width[div_sid] - (i * 64))
        if word_m != 0:
            _whole_stage_x_signal_shr(c, dst_sid, div_sid, shift)
            return
    for i in range(dst_words):
        c.wide_nba_val[dst_offset + i] = 0
        c.wide_nba_mask[dst_offset + i] = 0
...
cdef inline void _whole_stage_mod_const_shl(SimCtx *c, int dst_sid, int mod_sid, unsigned long long rhs_const, int shift) noexcept nogil:
    ...
    for i in range(src_words):
        ...
        if word_m != 0:
            _whole_stage_x_signal_shl(c, dst_sid, mod_sid, shift)
            return
    for i in range(q_digits):
        digit_index = q_digits - 1 - i
        word_index = digit_index >> 1
...
cdef inline void _whole_stage_mod_const_shr(SimCtx *c, int dst_sid, int mod_sid, unsigned long long rhs_const, int shift) noexcept nogil:
    ...
    for i in range(src_words):
        word_m = _sig_word_mask(c, mod_sid, i) & _word_mask64(c.width[mod_sid] - (i * 64))
        if word_m != 0:
            _whole_stage_x_signal_shr(c, dst_sid, mod_sid, shift)
            return
    for i in range(q_digits):
        digit_index = q_digits - 1 - i
        word_index = digit_index >> 1
...
cdef inline void _whole_stage_not_shr(SimCtx *c, int dst_sid, int signal_sid, int shift) noexcept nogil:
    ...
    cdef int src_words = c.wide_words[signal_sid] if c.wide_words[signal_sid] > 0 else 1
    cdef int out_words = dst_words if dst_words > 0 else 1
    cdef int i, src_index, remaining_w, src_remaining_w
    cdef int word_shift = shift >> 6
    cdef int bit_shift = shift & 63
...
cdef inline void _whole_stage_neg_shl(SimCtx *c, int dst_sid, int signal_sid, int shift) noexcept nogil:
    ...
    for i in range(src_words):
        ...
        if word_m != 0:
            _whole_stage_x_signal_shl(c, dst_sid, signal_sid, shift)
            return
    _whole_stage_const_sub_shl(c, dst_sid, (<unsigned long long>0), signal_sid, shift)

cdef inline void _whole_stage_neg_shr(SimCtx *c, int dst_sid, int signal_sid, int shift) noexcept nogil:
    cdef int src_words = c.wide_words[signal_sid] if c.wide_words[signal_sid] > 0 else 1
    cdef int i
    cdef unsigned long long word_m, src_tail_mask
    for i in range(src_words):
        ...
        word_m = _sig_word_mask(c, signal_sid, i) & src_tail_mask
        if word_m != 0:
            _whole_stage_x_signal_shr(c, dst_sid, signal_sid, shift)
            return
    _whole_stage_const_sub_shr(c, dst_sid, (<unsigned long long>0), signal_sid, shift)

cdef inline void _whole_stage_reduce_or_shift(SimCtx *c, int dst_sid, int signal_sid, int shift, int invert) noexcept nogil:
    cdef int dst_words = c.wide_words[dst_sid]
    cdef int out_words = dst_words if dst_words > 0 else 1
    cdef int src_words = c.wide_words[signal_sid] if c.wide_words[signal_sid] > 0 else 1
    cdef int i, src_remaining_w
...
cdef inline void _whole_stage_reduce_xor_shift(SimCtx *c, int dst_sid, int signal_sid, int shift, int invert) noexcept nogil:
    ...
    cdef int out_words = dst_words if dst_words > 0 else 1
    cdef int src_words = c.wide_words[signal_sid] if c.wide_words[signal_sid] > 0 else 1
    cdef int i, src_remaining_w, word_bits, parity = 0
...
cdef inline void _whole_stage_shl_signal(SimCtx *c, int dst_sid, int src_sid, int shift) noexcept nogil:
    cdef int dst_words = c.wide_words[dst_sid]
    cdef int src_words = c.wide_words[src_sid] if c.wide_words[src_sid] > 0 else 1
    cdef int i, src_index, remaining_w
    cdef int word_shift = shift >> 6
    cdef int bit_shift = shift & 63
...
cdef inline void _whole_stage_ternary_shl_signal(SimCtx *c, int dst_sid, int cond_sid, int true_sid, int false_sid, int shift) noexcept nogil:
    ...
    if cond_m == 0:
        if cond_v != 0:
            _whole_stage_shl_signal(c, dst_sid, true_sid, shift)
        else:
            _whole_stage_shl_signal(c, dst_sid, false_sid, shift)
        return
    if dst_words > 0:
        for i in range(dst_words):
            src_index = i - word_shift
...
cdef inline void _whole_stage_ternary_shr_signal(SimCtx *c, int dst_sid, int cond_sid, int true_sid, int false_sid, int shift) noexcept nogil:
    ...
    if cond_m == 0:
        if cond_v != 0:
            _whole_stage_shr_signal(c, dst_sid, true_sid, shift)
        else:
            _whole_stage_shr_signal(c, dst_sid, false_sid, shift)
        return
    if dst_words > 0:
        for i in range(dst_words):
            src_index = i + word_shift
...
cdef inline void _whole_stage_sar_signal(SimCtx *c, int dst_sid, int src_sid, int shift) noexcept nogil:
    ...
    if dst_words > 0:
        ...
        if shift >= 64:
            if sign_bit_m != 0:
                c.nba_val[dst_sid] = 0
                c.nba_mask[dst_sid] = <long long>_word_mask64(c.width[dst_sid])
            elif sign_bit_v != 0:
                c.nba_val[dst_sid] = <long long>_word_mask64(c.width[dst_sid])
                c.nba_mask[dst_sid] = 0
            else:
                c.nba_val[dst_sid] = 0
                c.nba_mask[dst_sid] = 0
