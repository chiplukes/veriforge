...
class VerilogFormatter:
    ...
    def _net(self, net: Net, ci: str) -> str:
        ...
        if net.initial_value and not net.dimensions:
            ...
            # -- only scalar/vector nets can. Any runtime default for an
            # array is established elsewhere (e.g. synchronous reset
            # logic); silently drop it here rather than emit illegal
            # `wire ... [0:N-1] = 0;`.
            parts.append("=")
    ...
    def _variable(self, var: Variable, ci: str) -> str:
        ...
        if var.initial_value and not var.dimensions:
            # Array/memory regs can't take an inline initializer in
            # Verilog -- only scalar/vector regs can. Any runtime default
            # for an array is established elsewhere (e.g. synchronous
            # reset logic); silently drop it here rather than emit
            # illegal `reg ... [0:N-1] = 0;`.
            parts.append("=")
