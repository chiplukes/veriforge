// Ibex RISC-V core Verilator testbench.
//
// Same memory/bus/halt logic as ../sim/testbench.v but adapted for Verilator:
//   - Clock and reset driven from C++ (no initial/always #delay)
//   - Unpacked array ICache ports tied off explicitly
//   - VCD tracing controlled from C++
//
...
module tb_verilator (
	...
	output logic [15:0] cycle_count_o
);
	...
	// ── Halt status (directly accessible from C++) ──────────────────
	reg halted_r = 0;
	reg [31:0] halt_code_r = 0;
	assign halted = halted_r;
	assign halt_code = halt_code_r;
	...
	always @(posedge clk) begin
		if (rst_n && data_req && data_we && data_addr == 32'h00003000) begin
			halted_r    <= 1;
			halt_code_r <= data_wdata;
		end
	end

	// ── Tie-off signals ─────────────────────────────────────────────
	wire [31:0] hart_id   = 32'h0;
	wire [31:0] boot_addr = 32'h0;
	wire        zero      = 1'b0;
	wire        one       = 1'b1;
	wire [14:0] zero15    = 15'h0;
	wire [3:0]  fetch_en  = 4'b0101;  // IbexMuBiOn

	// ── ICache tie-offs (ICache=0, these are unused) ────────────────
	// Unpacked array ports — must be explicitly tied off for Verilator
	wire [21:0] ic_tag_rdata [2];   // IC_NUM_WAYS=2, TagSizeECC=22
	wire [63:0] ic_data_rdata [2];  // IC_NUM_WAYS=2, LineSizeECC=64
	assign ic_tag_rdata[0]  = '0;
	assign ic_tag_rdata[1]  = '0;
	assign ic_data_rdata[0] = '0;
	assign ic_data_rdata[1] = '0;
