"""Run the imported common_cells plru_tree example.
...
FILES = [
    str(SCRIPT_DIR / "rtl" / "plru_tree.sv"),
    str(SCRIPT_DIR / "tb" / "plru_tree_tb_local.sv"),
]
MAX_TIME = 120
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("plru_tree_tb_local")
    if top is None:
        raise RuntimeError("Top module 'plru_tree_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("plru_tree example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing plru_tree example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_plru_tree_pcache")
    except Exception:
        traceback.print_exc()
        return 1
