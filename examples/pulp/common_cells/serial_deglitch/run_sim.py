"""Run the imported common_cells serial_deglitch example.
...
FILES = [
    str(SCRIPT_DIR / "rtl" / "serial_deglitch.sv"),
    str(SCRIPT_DIR / "tb" / "serial_deglitch_tb_local.sv"),
]
MAX_TIME = 140
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("serial_deglitch_tb_local")
    if top is None:
        raise RuntimeError("Top module 'serial_deglitch_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("serial_deglitch example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing serial_deglitch example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_serial_deglitch_pcache")
    except Exception:
        traceback.print_exc()
        return 1
