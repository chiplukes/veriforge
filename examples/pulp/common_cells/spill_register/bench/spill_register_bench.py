...
DUT_PATH = Path(__file__).resolve().parents[1] / "rtl" / "spill_register.sv"
# spill_register delegates to spill_register_flushable.
DEPS = [Path(__file__).resolve().parents[1] / "rtl" / "spill_register_flushable.sv"]
...
def drive_in(bench: Testbench) -> None:
    """Drive a known sequence into the spill register input.

    Domain: 'clk_i'
    """
    iface = bench.iface("in")
    iface.write([0xDE, 0xAD, 0xBE, 0xEF])


def expect_out(bench: Testbench) -> None:
    """Assert the spill register output replays the input sequence.

    Domain: 'clk_i'
    """
    iface = bench.iface("out")
    iface.expect_sequence([0xDE, 0xAD, 0xBE, 0xEF], timeout=200)
    print("spill_register passed: 4-beat round-trip [0xDE, 0xAD, 0xBE, 0xEF]")
