"""Run the imported common_cells lfsr_8bit example.
...
FILES = [
    str(SCRIPT_DIR / "rtl" / "lfsr_8bit.sv"),
    str(SCRIPT_DIR / "tb" / "lfsr_8bit_tb_local.sv"),
]
MAX_TIME = 120
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("lfsr_8bit_tb_local")
    if top is None:
        raise RuntimeError("Top module 'lfsr_8bit_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("lfsr_8bit example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing lfsr_8bit example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_lfsr_8bit_pcache")
    except Exception:
        traceback.print_exc()
        return 1
