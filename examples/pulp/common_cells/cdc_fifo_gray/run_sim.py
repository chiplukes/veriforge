"""Run the imported common_cells cdc_fifo_gray example.
...
FILES = [
    str(SCRIPT_DIR / "rtl" / "binary_to_gray.sv"),
    str(SCRIPT_DIR / "rtl" / "gray_to_binary.sv"),
    str(SCRIPT_DIR / "rtl" / "sync.sv"),
    str(SCRIPT_DIR / "rtl" / "spill_register_flushable.sv"),
    str(SCRIPT_DIR / "rtl" / "spill_register.sv"),
    str(SCRIPT_DIR / "rtl" / "cdc_fifo_gray.sv"),
    str(SCRIPT_DIR / "tb" / "cdc_fifo_gray_tb_local.sv"),
]
MAX_TIME = 280
...
def _make_step_sim(design, engine: str) -> Simulator:
    ...
    if top is None:
        raise RuntimeError("Top module 'cdc_fifo_gray_tb_local' not found")
...
def _run_engine_checks(design, engine: str) -> None:
    ...
    _expect(sim, "dst_data_o", FIRST_WORD, "destination data mismatch for first transfer")
...
def _run_engine(design, engine: str) -> int:
    ...
    try:
        ...
        print(f"  FAIL cdc_fifo_gray python {engine} checks: {exc}")
        print(f"  engine={engine} failed in {time.time() - t0:.2f}s")
        return 1

    print(f"  PASS cdc_fifo_gray python {engine} checks")
...
def main() -> int:
    ...
    if missing:
        print("cdc_fifo_gray example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing cdc_fifo_gray example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_cdc_fifo_gray_pcache")
    except Exception:
        traceback.print_exc()
        return 1
