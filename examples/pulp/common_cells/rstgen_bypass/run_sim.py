"""Run the imported common_cells rstgen_bypass example.
...
FILES = [
    ...
    str(SCRIPT_DIR / "tb" / "rstgen_bypass_tb_local.sv"),
]
MAX_TIME = 120
...
def _make_step_sim(design, engine: str) -> Simulator:
    top = design.get_module("rstgen_bypass_tb_local")
    if top is None:
        raise RuntimeError("Top module 'rstgen_bypass_tb_local' not found")
...
def _run_engine(design, engine: str) -> int:
    ...
    try:
        ...
        print(f"  FAIL rstgen_bypass python {engine} checks: {exc}")
        print(f"  engine={engine} failed in {time.time() - t0:.2f}s")
        return 1

    print(f"  PASS rstgen_bypass python {engine} checks")
...
def main() -> int:
    ...
    if missing:
        print("rstgen_bypass example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing rstgen_bypass example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_rstgen_bypass_pcache")
    except Exception:
        traceback.print_exc()
        return 1
