"""Run the imported common_cells delta_counter example.
...
RTL_FILE = SCRIPT_DIR / "rtl" / "delta_counter.sv"
TB_FILE = SCRIPT_DIR / "tb" / "delta_counter_tb_local.sv"
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("delta_counter_tb_local")
    if top is None:
        raise RuntimeError("Top module 'delta_counter_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("delta_counter example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing delta_counter example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_delta_counter_pcache")
    except Exception:
        traceback.print_exc()
        return 1
