"""Run the imported common_cells sub_per_hash example.

Run from the repository root:

    uv run python examples/pulp/common_cells/sub_per_hash/run_sim.py
"""
...
RTL_FILE = SCRIPT_DIR / "rtl" / "sub_per_hash.sv"
TB_FILE = SCRIPT_DIR / "tb" / "sub_per_hash_tb_local.sv"
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("sub_per_hash_tb_local")
    if top is None:
        raise RuntimeError("Top module 'sub_per_hash_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("sub_per_hash example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing sub_per_hash example...")
    t0 = time.time()
    try:
