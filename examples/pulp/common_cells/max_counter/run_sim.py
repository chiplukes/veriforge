"""Run the imported common_cells max_counter example.
...
FILES = [
    ...
    str(SCRIPT_DIR / "rtl" / "max_counter.sv"),
    str(SCRIPT_DIR / "tb" / "max_counter_tb_local.sv"),
]
MAX_TIME = 160
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("max_counter_tb_local")
    if top is None:
        raise RuntimeError("Top module 'max_counter_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("max_counter example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing max_counter example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_max_counter_pcache")
    except Exception:
        traceback.print_exc()
        return 1
