"""Run the imported common_cells trip_counter example.
...
FILES = [
    str(SCRIPT_DIR / "rtl" / "delta_counter.sv"),
    str(SCRIPT_DIR / "rtl" / "trip_counter.sv"),
    str(SCRIPT_DIR / "tb" / "trip_counter_tb_local.sv"),
]
MAX_TIME = 120
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("trip_counter_tb_local")
    if top is None:
        raise RuntimeError("Top module 'trip_counter_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("trip_counter example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing trip_counter example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_trip_counter_pcache")
    except Exception:
        traceback.print_exc()
        return 1
