"""Run the imported common_cells ring_buffer example.
...
FILES = [
    str(SCRIPT_DIR / "rtl" / "ring_buffer.sv"),
    str(SCRIPT_DIR / "tb" / "ring_buffer_tb_local.sv"),
]
MAX_TIME = 220
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("ring_buffer_tb_local")
    if top is None:
        raise RuntimeError("Top module 'ring_buffer_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("ring_buffer example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing ring_buffer example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_ring_buffer_pcache")
    except Exception:
        traceback.print_exc()
        return 1
