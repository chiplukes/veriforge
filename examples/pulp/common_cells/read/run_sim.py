"""Run the imported common_cells read example.
...
RTL_FILE = SCRIPT_DIR / "rtl" / "read.sv"
TB_FILE = SCRIPT_DIR / "tb" / "read_tb_local.sv"
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("read_tb_local")
    if top is None:
        raise RuntimeError("Top module 'read_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("read example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing read example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_read_pcache")
    except Exception:
        traceback.print_exc()
        return 1
