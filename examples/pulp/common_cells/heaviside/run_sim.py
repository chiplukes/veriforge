"""Run the imported common_cells heaviside example.
...
RTL_FILE = SCRIPT_DIR / "rtl" / "heaviside.sv"
TB_FILE = SCRIPT_DIR / "tb" / "heaviside_tb_local.sv"
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("heaviside_tb_local")
    if top is None:
        raise RuntimeError("Top module 'heaviside_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("heaviside example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing heaviside example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_heaviside_pcache")
    except Exception:
        traceback.print_exc()
        return 1
