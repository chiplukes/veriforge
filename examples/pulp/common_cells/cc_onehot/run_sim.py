"""Run the imported common_cells cc_onehot example.
...
RTL_FILE = SCRIPT_DIR / "rtl" / "cc_onehot.sv"
TB_FILE = SCRIPT_DIR / "tb" / "cc_onehot_tb_local.sv"
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("cc_onehot_tb_local")
    if top is None:
        raise RuntimeError("Top module 'cc_onehot_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("cc_onehot example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing cc_onehot example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_cc_onehot_pcache")
    except Exception:
        traceback.print_exc()
        return 1
