"""Run the imported common_cells unread example.
...
RTL_FILE = SCRIPT_DIR / "rtl" / "unread.sv"
TB_FILE = SCRIPT_DIR / "tb" / "unread_tb_local.sv"
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("unread_tb_local")
    if top is None:
        raise RuntimeError("Top module 'unread_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("unread example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing unread example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_unread_pcache")
    except Exception:
        traceback.print_exc()
        return 1
