"""Run the imported common_cells gray_to_binary example.
...
RTL_FILE = SCRIPT_DIR / "rtl" / "gray_to_binary.sv"
TB_FILE = SCRIPT_DIR / "tb" / "gray_to_binary_tb_local.sv"
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("gray_to_binary_tb_local")
    if top is None:
        raise RuntimeError("Top module 'gray_to_binary_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("gray_to_binary example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing gray_to_binary example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_gray_to_binary_pcache")
    except Exception:
        traceback.print_exc()
        return 1
