"""Run the imported common_cells lzc example.
...
RTL_FILE = SCRIPT_DIR / "rtl" / "lzc.sv"
TB_FILE = SCRIPT_DIR / "tb" / "lzc_tb_local.sv"
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("lzc_tb_local")
    if top is None:
        raise RuntimeError("Top module 'lzc_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("lzc example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing lzc example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_lzc_pcache")
    except Exception:
        traceback.print_exc()
        return 1
