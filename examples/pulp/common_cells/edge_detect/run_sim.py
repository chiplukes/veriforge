"""Run the imported common_cells edge_detect example.
...
FILES = [
    ...
    str(SCRIPT_DIR / "rtl" / "edge_detect.sv"),
    str(SCRIPT_DIR / "tb" / "edge_detect_tb_local.sv"),
]
MAX_TIME = 140
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("edge_detect_tb_local")
    if top is None:
        raise RuntimeError("Top module 'edge_detect_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("edge_detect example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing edge_detect example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_edge_detect_pcache")
    except Exception:
        traceback.print_exc()
        return 1
