"""Run the imported common_cells shift_reg example.
...
FILES = [
    str(SCRIPT_DIR / "rtl" / "shift_reg_gated.sv"),
    str(SCRIPT_DIR / "rtl" / "shift_reg.sv"),
    str(SCRIPT_DIR / "tb" / "shift_reg_tb_local.sv"),
]
MAX_TIME = 140
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("shift_reg_tb_local")
    if top is None:
        raise RuntimeError("Top module 'shift_reg_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("shift_reg example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing shift_reg example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_shift_reg_pcache")
    except Exception:
        traceback.print_exc()
        return 1
