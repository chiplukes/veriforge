"""Run the imported common_cells binary_to_gray example.
...
RTL_FILE = SCRIPT_DIR / "rtl" / "binary_to_gray.sv"
TB_FILE = SCRIPT_DIR / "tb" / "binary_to_gray_tb_local.sv"
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("binary_to_gray_tb_local")
    if top is None:
        raise RuntimeError("Top module 'binary_to_gray_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("binary_to_gray example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing binary_to_gray example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_binary_to_gray_pcache")
    except Exception:
        traceback.print_exc()
        return 1
