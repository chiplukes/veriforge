"""Run the imported common_cells counter example.
...
FILES = [
    ...
    str(SCRIPT_DIR / "rtl" / "counter.sv"),
    str(SCRIPT_DIR / "tb" / "counter_tb_local.sv"),
]
MAX_TIME = 120
...
def _run_engine(design, engine: str) -> int:
    ...
    top = design.get_module("counter_tb_local")
    if top is None:
        raise RuntimeError("Top module 'counter_tb_local' not found")
...
def main() -> int:
    ...
    if missing:
        print("counter example is scaffolded but not runnable yet.")
        print("Missing files:")
        for path in missing:
    ...
    print("Parsing counter example...")
    t0 = time.time()
    try:
        design = parse_files(FILES, preprocess=True, cache_dir=SCRIPT_DIR / "_vtc_counter_pcache")
    except Exception:
        traceback.print_exc()
        return 1
