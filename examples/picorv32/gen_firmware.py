"""Generate RV32I instruction test firmware for PicoRV32.
...
Memory layout (1024 words = 4096 bytes, byte-addressed):
...
# Memory: 1024 words = 4096 bytes
# Addresses must have low 12 bits < 0x800 for clean ADDI (no sign-extension).
RESULT_BASE = 0x600  # Word 384 — code uses up to ~264 words
...
# Test group names (used by testbench/runner)
TEST_NAMES = [
    "LUI/AUIPC",
    "JAL/JALR",
    "BRANCH",
...
class Program:
    ...
    def pc(self):
        """Current PC (byte address of next instruction)."""
        return len(self.code) * 4

    def offset_to(self, label_name):
        """Byte offset from current PC to a label."""
        return self.labels[label_name] - self.pc()

    def write_hex(self, path, total_words=1024):
...
def assert_eq(prog, ra, rb):
    """If ra != rb, branch to self (hang). Use only for must-pass checks."""
...
def build_program():
    ...
    # ── Setup ────────────────────────────────────────────────────────
    # Initialize result area to 0 (fail) — overwritten on pass
    ...
    # LUI: T1 should get 0x12345000
    p.emit(LUI(T1, 0x12345))
    # Verify upper bits: shift right 20 → top 12 bits
    p.emit(SRLI(T2, T1, 20))  # T2 = 0x123
    p.emit(LI(T3, 0x123))
    assert_eq(p, T2, T3)
    # Verify middle bits: shift right 12, mask to low 8 bits
    p.emit(SRLI(T2, T1, 12))  # T2 = 0x12345
    p.emit(ANDI(T2, T2, 0xFF))  # T2 = 0x45
    p.emit(LI(T3, 0x45))
    assert_eq(p, T2, T3)
    # Verify low 12 bits are 0
    p.emit(ANDI(T2, T1, 0x7FF))  # low 11 bits
    assert_eq(p, T2, ZERO)
    ...
    pc_jal = p.pc()
    p.emit(JAL(RA, 8))  # jump +8 (skip next insn), RA = pc_jal + 4
    p.emit(JAL(ZERO, 0))  # HANG — should be skipped
    # Landed here: verify RA = pc_jal + 4
    p.emit(LI(T1, pc_jal + 4))
    assert_eq(p, RA, T1)

    # JALR: compute target address, jump to it
    p.emit(AUIPC(T1, 0))  # T1 = PC of this insn (X)
    p.emit(ADDI(T1, T1, 16))  # T1 = X + 16 (skip JALR + HANG to land after)
    p.emit(JALR(RA, T1, 0))  # jump to T1 (X+16)
    p.emit(JAL(ZERO, 0))  # HANG — should be skipped
    ...
    p.emit(BNE(T0, T1, 8))
    p.emit(JAL(ZERO, 0))
    # BLT: 5 < 10 signed → taken
    p.emit(BLT(T0, T1, 8))
    p.emit(JAL(ZERO, 0))
    # BGE: 10 >= 5 signed → taken
    p.emit(BGE(T1, T0, 8))
    p.emit(JAL(ZERO, 0))
    # BLTU: 5 < 10 unsigned → taken
    p.emit(BLTU(T0, T1, 8))
    p.emit(JAL(ZERO, 0))
    # BGEU: 0xFFFFFFFF >= 10 unsigned → taken
    p.emit(BGEU(T3, T1, 8))
    p.emit(JAL(ZERO, 0))
    ...
    # ================================================================
    # TEST 4: ALU immediate (ADDI, ORI, ANDI, XORI)
    # ================================================================
    p.label("test_alu_imm")
    p.emit(LI(T0, 100))
    p.emit(ADDI(T1, T0, 23))  # 123
    p.emit(LI(T2, 123))
    assert_eq(p, T1, T2)

    p.emit(ADDI(T1, T0, -50))  # 50
    p.emit(LI(T2, 50))
    assert_eq(p, T1, T2)

    p.emit(LI(T0, 0x0F0))
    p.emit(ORI(T1, T0, 0x00F))  # 0x0FF
    p.emit(LI(T2, 0xFF))
    assert_eq(p, T1, T2)

    p.emit(LI(T0, 0x1FF))
    p.emit(ANDI(T1, T0, 0x0F0))  # 0x0F0
    p.emit(LI(T2, 0xF0))
    assert_eq(p, T1, T2)

    p.emit(LI(T0, 0xFF))
    p.emit(XORI(T1, T0, 0x0F))  # 0xF0
    p.emit(LI(T2, 0xF0))
    assert_eq(p, T1, T2)
    ...
    # ================================================================
    # TEST 5: ALU register (ADD, SUB, OR, AND, XOR)
    # ================================================================
    p.label("test_alu_reg")
    p.emit(LI(T0, 30))
    p.emit(LI(T1, 12))

    p.emit(ADD(T2, T0, T1))  # 42
    p.emit(LI(T3, 42))
    assert_eq(p, T2, T3)

    p.emit(SUB(T2, T0, T1))  # 18
    p.emit(LI(T3, 18))
    assert_eq(p, T2, T3)

    p.emit(LI(T0, 100))
    p.emit(LI(T1, -25))
    p.emit(ADD(T2, T0, T1))  # 75
    p.emit(LI(T3, 75))
    assert_eq(p, T2, T3)

    p.emit(LI(T0, 0x0F0))
    p.emit(LI(T1, 0x00F))
    p.emit(OR(T2, T0, T1))  # 0x0FF
    p.emit(LI(T3, 0xFF))
    assert_eq(p, T2, T3)

    p.emit(LI(T0, 0x1FF))
    p.emit(LI(T1, 0x0F0))
    p.emit(AND(T2, T0, T1))  # 0x0F0
    p.emit(LI(T3, 0xF0))
    assert_eq(p, T2, T3)

    p.emit(LI(T0, 0xFF))
    p.emit(LI(T1, 0x0F))
    p.emit(XOR(T2, T0, T1))  # 0xF0
    p.emit(LI(T3, 0xF0))
    assert_eq(p, T2, T3)
    ...
    # ================================================================
    # TEST 6: SHIFT (SLLI, SRLI, SRAI, SLL, SRL, SRA)
    # ================================================================
    p.label("test_shift")
    p.emit(LI(T0, 1))
    p.emit(SLLI(T1, T0, 4))  # 16
    p.emit(LI(T2, 16))
    assert_eq(p, T1, T2)

    p.emit(LI(T0, 16))
    p.emit(SRLI(T1, T0, 2))  # 4
    p.emit(LI(T2, 4))
    assert_eq(p, T1, T2)

    p.emit(LI(T0, -16))  # 0xFFFFFFF0
    p.emit(SRAI(T1, T0, 2))  # -4 = 0xFFFFFFFC
    p.emit(LI(T2, -4))
    assert_eq(p, T1, T2)

    p.emit(LI(T0, 1))
    p.emit(LI(T1, 8))
    p.emit(SLL(T2, T0, T1))  # 256
    p.emit(LI(T3, 256))
    assert_eq(p, T2, T3)

    p.emit(LI(T0, 256))
    p.emit(LI(T1, 4))
    p.emit(SRL(T2, T0, T1))  # 16
    p.emit(LI(T3, 16))
    assert_eq(p, T2, T3)

    p.emit(LI(T0, -128))
    p.emit(LI(T1, 3))
    p.emit(SRA(T2, T0, T1))  # -16
    p.emit(LI(T3, -16))
    assert_eq(p, T2, T3)
    ...
    p.emit(SLT(T2, T0, T1))  # 5 < 10 → 1
    p.emit(LI(T4, 1))
    assert_eq(p, T2, T4)

    p.emit(SLT(T2, T1, T0))  # 10 < 5 → 0
    assert_eq(p, T2, ZERO)

    p.emit(SLT(T2, T3, T0))  # -1 < 5 → 1
    p.emit(LI(T4, 1))
    assert_eq(p, T2, T4)

    p.emit(SLTU(T2, T0, T3))  # 5 < 0xFFFFFFFF → 1
    p.emit(LI(T4, 1))
    assert_eq(p, T2, T4)

    p.emit(SLTI(T2, T0, 10))  # 5 < 10 → 1
    p.emit(LI(T4, 1))
    assert_eq(p, T2, T4)

    p.emit(SLTIU(T2, T0, 10))  # 5 < 10 → 1
    p.emit(LI(T4, 1))
    assert_eq(p, T2, T4)
    ...
    p.emit(LUI(T0, 0xAAAAA))  # T0 = 0xAAAAA000
    p.emit(LUI(T1, 0x55555))  # T1 = 0x55555000

    # OR
    p.emit(OR(T2, T0, T1))
    p.emit(LUI(T3, 0xFFFFF))  # 0xFFFFF000
    assert_eq(p, T2, T3)

    # AND
    p.emit(AND(T2, T0, T1))  # 0x00000000
    assert_eq(p, T2, ZERO)
    ...
    # NOT via XORI with -1: ~0xAAAAA000 = 0x55555FFF
    p.emit(XORI(T2, T0, -1))
    p.emit(LUI(T3, 0x55556))
    p.emit(ADDI(T3, T3, -1))  # 0x55556000 - 1 = 0x55555FFF
    assert_eq(p, T2, T3)
    ...
    p.emit(LI(T0, SCRATCH))
    p.emit(SW(ZERO, T0, 0))  # clear scratch word

    # SB 0xAB at byte 0
    p.emit(LI(T1, 0xAB))  # 0xAB < 0x800, so T1 = 0x000000AB
    p.emit(SB(T1, T0, 0))

    # LBU should return 0x000000AB
    p.emit(LBU(T2, T0, 0))
    p.emit(LI(T3, 0xAB))
    assert_eq(p, T2, T3)

    # LB: 0xAB has bit 7 set → sign-extends to 0xFFFFFFAB = -85
    p.emit(LB(T2, T0, 0))
    p.emit(LI(T3, -85))
    assert_eq(p, T2, T3)
    ...
    # Word read: little-endian byte0=AB, byte1=34, byte2-3=00
    # Expected: 0x000034AB
    p.emit(LW(T2, T0, 0))
    p.emit(LUI(T3, 0))
    p.emit(LI(T3, 0x34))
    p.emit(SLLI(T3, T3, 8))
    p.emit(ORI(T3, T3, 0xAB))  # T3 = 0x000034AB
    assert_eq(p, T2, T3)
    ...
    p.emit(LI(T0, SCRATCH))
    p.emit(SW(ZERO, T0, 8))  # clear scratch+8

    # Build 0x1234 and store as halfword
    p.emit(LI(T1, 0x12))
    p.emit(SLLI(T1, T1, 8))  # 0x1200
    p.emit(ORI(T1, T1, 0x34))  # 0x1234
    ...
    # Store 0x8765 at offset 10 (high bit set)
    p.emit(LI(T1, 0x87))
    p.emit(SLLI(T1, T1, 8))
    p.emit(ORI(T1, T1, 0x65))  # T1 = 0x00008765
    p.emit(SH(T1, T0, 10))

    # LH: sign-extend → should be negative
    p.emit(LH(T2, T0, 10))
    p.emit(SLT(T3, T2, ZERO))  # negative? → 1
    p.emit(LI(T4, 1))
    assert_eq(p, T3, T4)

    # LHU: zero-extend → should be positive
    p.emit(LHU(T2, T0, 10))
    p.emit(SLT(T3, T2, ZERO))  # negative? → 0
    assert_eq(p, T3, ZERO)

    set_pass(p, TEST_MEMORY_HALF)

    # ================================================================
    # DONE: trigger EBREAK → trap
    # ================================================================
    p.label("done")
    p.emit(EBREAK())
    ...
    max_code_words = RESULT_BASE // 4  # code must not overlap results
...
def main():
    ...
    # Also print a quick disassembly summary
    print("\nLabels:")
    for name, addr in sorted(p.labels.items(), key=lambda x: x[1]):
        print(f"  0x{addr:03x}: {name}")

    # Verify some instructions encode correctly
    _verify_encodings()


def _verify_encodings():
    """Quick sanity check on a few encodings."""
    # NOP = ADDI x0, x0, 0 = 0x00000013
    assert NOP() == 0x00000013, f"NOP: {NOP():#010x}"
    # EBREAK = 0x00100073
    assert EBREAK() == 0x00100073, f"EBREAK: {EBREAK():#010x}"
    # ADDI x1, x0, 1020 = 0x3fc00093
