"""PicoRV32 fast RV32I instruction test — uses batch_run for pure-C execution.
...
Usage:
    uv run python examples/picorv32/run_fast.py
"""
...
def main() -> int:
    ...
    # ── Initialize: execute initial blocks and settle ────────────────
    print("\nInitializing (settle combinational logic)...")
    t0 = time.time()
    sim.run(max_time=0)
    t_init = time.time() - t0
    ...
    # Release reset after RESET_CYCLES clocks
    events = [(RESET_CYCLES, "resetn", 1)]
    ...
    if sim.display_output:
        ...
        for line in sim.display_output[:50]:
            print(f"  {line}")
        if len(sim.display_output) > 50:
            print(f"  ... ({len(sim.display_output) - 50} more lines)")

    # ── Key signals ──────────────────────────────────────────────────
    trap_val = sim.read("trap")

    print(f"\ntrap = {trap_val}")
    print(f"Sim time = {sim.time}")
