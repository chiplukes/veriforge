"""PicoRV32 cosim validation: compare reference engine vs Icarus Verilog.

Uses run_cycle_by_cycle to step both simulators in lockstep and report
the first divergence.

Usage:
    uv run python examples/picorv32/cosim_validate.py
"""
...
RTL = [os.path.join(RTL_DIR, "picorv32.v")]
...
# PicoRV32 uses active-LOW reset (resetn: starts 0, release to 1)
# TRACE_* are debug-only signals with width differences in VCD; not functional
result = cosim.run_cycle_by_cycle(
    ...
    sim_files=SIM_FILES,
    ignore_signals={"uut.TRACE_ADDR", "uut.TRACE_BRANCH", "uut.TRACE_IRQ"},
    verbose=True,
)
