...
> **This is a frozen historical snapshot**, split out from `known_issues.md`
> in August 2026 (per `architecture_review_2026-07.md` item 9's "known_issues.md
> holds only defects" plan) once that file had grown to be almost entirely
> resolved/historical content. It preserves the full investigative record —
> root causes, Icarus repros, fix methodology, "waves" of related discoveries —
> for every defect that was open at some point during this project's
> development. **Most of what's below is RESOLVED**; each entry's own
> `**Status**` line is the source of truth for whether it's still relevant.
>
> For currently open defects, see [`known_issues.md`](known_issues.md) instead —
> that file is kept short and current on purpose. A few items below were still
> open as of the split and were copied forward into `known_issues.md` in
> trimmed form (the vm-fast `**` wide-operand gap, buried inside the "ternary/
> context-determined-operator codegen" entry below; the wide function port/
> return and wide-posedge gaps, formerly under "Compiled engine: 64-bit signal
> width limit" below). Treat this file as read-only reference, not a place to
> add new entries — new defects belong in `known_issues.md`.
...
**Status**: Essentially resolved (re-assessed August 2026 — this entry was
stale). One narrow, deliberately-unsupported architectural gap remains.
**Found**: Ibex simulation work (March 2026)
**Severity**: Was High when found (blocked real-world AXI/wide-bus
designs); now Low (the one remaining gap fails loudly with a clear error
at codegen time rather than silently producing wrong results).

The compiled Cython engine originally stored all signals as C `long long`
(64-bit), which truncated wider values. `notes/simulation/
wide_signal_coverage.md` tracks per-operation coverage in detail — as of
this re-assessment, every operator/statement-form row in that matrix is
✅ (bitwise, arithmetic, shifts, comparisons, reductions, logical ops,
selects/concat/replication, ternary, blocking/NBA/continuous assign,
memory element read/write, struct fields, edge-detection guards), backed
by real cross-engine test coverage (`tests/test_sim/compiled/
test_wide_ops.py`, `test_cross_validation.py`, `test_memories.py` — 442
passed on re-verification; `test_external_io_slow.py::
TestWideSignalExternalIO`, 5563 tests, not run as part of this
re-verification since it's excluded from the default suite, see that
file's own slow-test marker). The "internal compiled expression,
assignment, NBA, and dirty-propagation codegen still has remaining
single-word assumptions" language this entry used to carry was itself
stale — re-verified against Icarus for a >64-bit unary `~`/`-` evaluated
in a wider destination context (the specific case
`notes/plans/architecture_review_2026-07.md`'s item 8 once flagged as a
live bug) and it already matches Icarus exactly on all four engines; see
that item's own entry, now marked resolved, for why the original
diagnosis there was backwards.

**One remaining gap, by design, not a bug**: user-defined functions with
a wide (>64-bit) port or return type raise `NotImplementedError` at
codegen time (`test_compiled_function_wide_port_raises`) rather than
being silently truncated. The `_user_func_XXX` call ABI is hardcoded to
a single native `long long` per argument/return; supporting a wide
port/return would need a genuine multi-word call-ABI redesign (argument
and return-value passing), not a routing fix like the rest of the
coverage matrix. A wide *argument expression* passed to a narrow port
already works correctly. Reference/VM/VM-fast are unaffected (they use
Python `int` throughout and have no such limit). Not attempted here —
this is a real architectural project, not a bug fix, and should be
scoped as its own work-plan item if it's ever needed (no design
encountered so far actually requires it).

See `notes/plans/architecture_review_2026-07.md` and
`notes/simulation/wide_signal_coverage.md` for the full coverage matrix
