# Consolidated Work Plan — July 2026 (Archive)

> **This is a frozen historical snapshot** of the full work plan,
> including every completed item's step-by-step plan and execution
> "Result" notes, split out in August 2026 once tiers 1-3 and items
> 4.1-4.2 were all done and the live file had grown to ~2500 lines of
> mostly-finished work. See [`work_plan_2026-07.md`](work_plan_2026-07.md)
> for the current, short version: a completion checklist for everything
> below plus full detail for the items that are actually still open
> (4.3, 4.4, 4.5). Treat this file as read-only reference.
...
- User-defined `function` call fuzzing is now implemented as Phase 6,
  below (was deferred here; promoted once Phase 5 landed clean).
...
**Phase 6 — user-defined `function` call
fuzzing**: every phase so far generates only `$signed`/`$unsigned` as
"calls" in an expression tree; real Verilog `function`s (declared once,
called from many expression positions — the standard way real RTL
factors out reusable combinational logic, e.g. parity/priority-encode/
saturate helpers) have never been generated by either fuzzer. All four
engines already IMPLEMENT user-defined function calls (this isn't a
"does it even work" gap like `forever`/`repeat` was) —
`sim/evaluator.py:1169`'s `_eval_user_function`
(fresh `local_signals` scope per call: parent copy + port bindings + a
return variable named after the function, executed via the statement
interpreter, then read back), `sim/vm/compiler.py:1667`'s
`_compile_user_function` (dispatched via a `_function_map`), and
`sim/compiled/_expr_emitter.py:2278`'s `_emit_func_call` (falls through
system builtins to a real generated Cython `_user_func_{name}` function,
built from `module.functions` in `_gen_sections.py`). Only deterministic,
hand-written coverage exists today (`tests/test_sim/test_function_task.py`,
`test_compiled_latent_risks.py`) — never fuzzed, and the model already
supports it cleanly: `FunctionCall` (`model/expressions.py:348`) carries
`is_system: bool` to distinguish `$signed`-style calls from user calls,
and `FunctionDecl`/`TaskDecl` (`model/functions.py:34,104`) carry
`is_automatic`, `ports`, `locals`, `body`, `return_range`/`return_kind`.

**Goal**: generate expression trees that call a small, FIXED pool of
user-defined functions (declared once per batch module, analogous to how
`FIXED_SIGNALS` are shared ports) from arbitrary expression positions,
with randomly-generated ARGUMENT expressions — exercising argument
width/signedness propagation into a real call site and the call's own
return-value width/signedness propagating back out, the one call shape
(`$signed`/`$unsigned`) fuzzed so far can't stand in for.

**Design**:
1. A small fixed function pool (recommend 3-4 functions, 1-2 args each,
   mixed signed/unsigned and mixed widths from the existing `_WIDTHS`
   list, mirroring `FIXED_SIGNALS`' own diversity) declared ONCE at
   module scope in `_build_batch_module` (which currently has no
   function-declaration section at all — this is the one genuinely new
   piece of module-assembly plumbing this phase needs), shared across
   every case in the batch.
2. Function BODIES are FIXED, simple, single-statement (`func_name =
   <fixed expr of the ports>;`), NOT randomly generated and NOT
   containing calls to each other or to themselves — deliberately
   out of scope, for two independent reasons: (a) none of
   `evaluator.py`/`compiler.py`/`_gen_sections.py`'s call-site dispatch
   logic references `is_automatic` anywhere (confirmed by grep) --
   recursion/reentrancy
   semantics for automatic vs static functions are effectively
   engine-specific/unverified, not a spec-driven behavior this harness
   is equipped to arbitrate correctly yet, so generating recursive calls
   risks fuzzing an implementation-detail gap rather than a genuine
   correctness question; (b) keeping bodies fixed bounds this phase
   to exactly the new thing being tested (CALL SITE argument/return
   propagation), matching every prior phase's "characterize the
   simplest new construct shape first" discipline.
3. New leaf/expr option in `test_differential.py`'s `_gen_leaf`/
   `_gen_expr`: a small registry of `(name, arg_widths_signs,
   return_width, return_signed)` tuples (populated from the fixed pool
   above), generating `funcname(arg_expr_1, ...)` where each argument is
   itself `_gen_expr(rng, depth - 1, extra_signals)` (recursing at
   reduced depth, mirroring how nested unary/binary operators already
   bound tree growth) — plugs in alongside the existing `$signed`/
   `$unsigned` cast generation, not replacing it.
4. Purely additive for `test_differential_statements.py`: since a
   function call is an expression-position construct (like `$signed`),
   once `_gen_leaf`/`_gen_expr` support it, the statement fuzzer gets it
   for free through its existing `extra_signals`/`td._gen_expr`
   threading (the same plumbing Phase 5 built) — no interaction needed
   with `_SeqState`/`_LoopState`, no new statement-level generator code.
5. `task` calls excluded: tasks return nothing (no expression-position
   use) and may contain timing controls, a statement-level/non-
   combinational concern outside this harness's scope — mirrors the
   `forever`/`repeat` deprioritization rationale already established.

**Open questions** (resolve before/while implementing): exact fixed-pool
size/diversity (proposed: 3-4 functions, 1-2 args, matching `_WIDTHS`'
width spread and a signed/unsigned mix); whether function bodies should
ever use MULTIPLE internal statements/local variables (recommend
starting single-expression `func = expr;` bodies only, revisiting
multi-statement bodies as later future work if single-expression proves
insufficient to find anything new); `automatic` vs default (static)
keyword choice for the fixed pool (likely immaterial without recursion,
but pick one explicitly and note why — default/static matches what none
of the four engines special-case today).

**Steps**: (1) design and land the fixed function pool + module-scope
declaration plumbing in `_build_batch_module`; (2) add the call-site
leaf/expr generator, verify `test_differential.py`'s OWN existing suite
stays byte-identical when the new option's probability is 0 (no-op
check, mirroring Phase 5's `extra_signals` verification); (3)
default-scale run green for both fuzzers; (4) stress-test at 150 cases
...
**Result** (August 2026): Built largely as specified, with one design
change made mid-implementation (confirmed with the user before
proceeding): `FIXED_FUNCTIONS`/`_func_declaration` landed in
`test_differential.py` as planned (shared with, not duplicated by, the
new file), and `_gen_expr` gained the planned `callables` parameter
(no-op by default, verified byte-identical for every pre-existing call
site) — but the call-site generator and narrower (64-bit, not 96-bit)
destination width live in a genuinely NEW file,
`test_differential_functions.py`, rather than being folded into
`test_differential.py`/`test_differential_statements.py`'s own shared
96-bit-destination harness as originally scoped. Reason: the compiled
engine cannot handle a `FunctionCall` node anywhere in an RHS tree
feeding a >64-bit destination (confirmed directly, raises
`NotImplementedError`) — plugging function calls into either existing
fuzzer's shared harness would make any `VERIFORGE_DIFF_COMPILED=1`/
`VERIFORGE_DIFF_STMT_COMPILED=1` run crash outright the moment a single
generated case happened to contain a call, the identical constraint
that already excluded `**` into its own dedicated `test_power_
operator.py`. `test_differential_statements.py` was therefore NOT
touched at all — function calls remain purely an expression-position
concern, and the new file's own default (non-compiled) run is fully
green, so this did not block the fuzzer's own value.

Stress-testing at 150 cases across the 8-seed rotation with
`VERIFORGE_DIFF_FUNC_COMPILED=1` surfaced fifteen distinct bugs across
all four engines — reference (2: argument-to-port binding never
narrowed/never evaluated at the port's own context width; self-
determined-width estimator hardcoded a call's own width to a generic
`32` fallback instead of the function's real return width), `sim/vm/
compiler.py` (3: the identical argument-width and `32`-fallback gaps,
plus a nested-same-function-call depth-counter collision), `vm-fast`
(1: `OP_SIGN_EXT` never demoting a wide-flagged source back to the
narrow stack representation when narrowing to <=64 bits), and compiled
(9: hardcoded argument width; no mask/x-z channel in the function-call
ABI at all; `*`/`/`/`%`'s mask formula missing the "any x anywhere ->
x everywhere" rule, a genuine pre-existing gap unrelated to function
calls; a ternary arithmetic branch's mask queried at the wrong width;
no wide-emitter support for user functions at all; reductions silently
truncating >64-bit operands via native `long long`, a second genuine
pre-existing gap unrelated to function calls, whose fix itself exposed
...
reduction-truncation gap) are confirmed, via minimal function-free
repros, to be GENUINELY PRE-EXISTING and unrelated to function calls at
all — Phase 6's fuzzer just happened to be the first to reach them.
Fixing the reduction gap in turn surfaced one more: the SAME "wide
computed value reached from a narrow context" class of bug also affects
OTHER node types (confirmed for `TernaryOp` conditions, via a function-
call-free repro) that the reduction-specific fix doesn't cover — this
residual gap is deliberately deferred as a known, separately-scoped
follow-up (mirrors item 2.7 sub-item 4's own "compiled engine's 64-bit
width limit is only partially resolved" framing) rather than expanding
this phase into an open-ended compiled-engine hardening project;
confirmed with the user before stopping. `test_differential_functions.py`'s
default (non-compiled) run is fully green across all 8 seeds regardless.

Verified via all 8 expression-tree fuzzer seeds and all 14 statement-
fuzzer seeds (both with their own compiled flag set, confirming the
two existing fuzzers are unaffected by every shared-file change above),
all 8 function-call fuzzer seeds with no compiled (fully green) and
with `VERIFORGE_DIFF_FUNC_COMPILED=1` (green except the one documented
residual gap), `test_function_task.py` (29 passed, unaffected),
`test_power_operator.py` (60 passed, 1 xfail, unaffected), and a final
