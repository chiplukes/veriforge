...
**Phase 6 plumbing** (work plan item 3.4): `_gen_expr`'s optional
`callables` parameter and `FIXED_FUNCTIONS` below exist to let
`test_differential_functions.py` generate calls to a small fixed pool of
user-defined Verilog `function`s from arbitrary expression positions.
`callables` defaults to `()`, a no-op for every call site in THIS file
(never passed here) -- verified byte-identical to before this parameter
existed. It lives here, not in the new file, so the SAME `_gen_expr`
implementation (and its existing `extra_signals` machinery) is reused
rather than duplicated -- exactly the same reasoning as `extra_signals`
itself (added for `test_differential_statements.py`, Phase 5). See
`test_differential_functions.py`'s own module docstring for why function
calls get their OWN dedicated file with a narrower destination width
instead of plugging into this file's or `test_differential_statements.py`'s
shared 96-bit-destination harness: the compiled engine cannot yet handle a
`FunctionCall` node anywhere in an RHS tree feeding a >64-bit destination
(confirmed directly -- raises `NotImplementedError`; a pre-existing,
separately-scoped gap, work plan item 2.7 sub-item 4), the identical
constraint that already excluded `**` from both fuzzers in favor of its
own dedicated `test_power_operator.py`.
"""
...
class _FuncSpec:
    """A user-defined Verilog `function` available for `_gen_expr`'s
    `callables`-gated "call" node kind to reference (Phase 6, work plan
    item 3.4). `arg_widths`/`arg_signed` describe each parameter's
    declared type -- an argument EXPRESSION generated for a call doesn't
    need to match its parameter's width itself (same as any other
    binary-op operand generated by this file), Verilog's normal
    argument-passing width conversion handles that at the call site,
    exactly the thing worth fuzzing. `body` is a single fixed statement
    (not randomly generated) assigning the function's own name -- see
    `FIXED_FUNCTIONS`'s docstring for why bodies are deliberately fixed
    and non-recursive.
    """

    name: str
    arg_widths: tuple[int, ...]
    arg_signed: tuple[bool, ...]
    ret_width: int
    ret_signed: bool
    body: str


# A small FIXED pool of simple, non-recursive functions (bodies are never
# randomly generated -- this phase fuzzes CALL SITES, i.e. how randomly
# generated argument expressions and a call's own return value propagate
# width/signedness into and out of a real function boundary, not the
# space of possible function bodies). Recursion/self-calls are
# deliberately excluded too: none of `sim/evaluator.py`/`sim/vm/
# compiler.py`/`sim/compiled/_gen_sections.py`'s call-site dispatch logic
# references a function's `is_automatic` flag anywhere (confirmed by
# direct grep) -- recursion/reentrancy semantics for automatic vs static
# functions are effectively engine-specific/unverified, not a spec-driven
# behavior this harness is equipped to arbitrate correctly yet, so a
# generated recursive call would risk fuzzing an implementation-detail
# gap rather than a genuine correctness question. Every width here is
# <=64 bits, matching `test_differential_functions.py`'s own narrower
# destination width (see that file's docstring for why) -- keeping the
# function POOL itself narrow too avoids re-triggering the exact same
# compiled-engine wide-RHS gap one level down, inside the function body's
# own internal assignment to its return variable.
FIXED_FUNCTIONS: list[_FuncSpec] = [
    _FuncSpec("fn_add8", (8, 8), (False, False), 8, False, "fn_add8 = a + b;"),
    _FuncSpec("fn_sub16s", (16, 16), (True, True), 16, True, "fn_sub16s = a - b;"),
    _FuncSpec("fn_sel1", (1, 63), (False, False), 63, False, "fn_sel1 = a ? b : ~b;"),
    _FuncSpec("fn_xor64s", (64, 8), (True, False), 64, True, "fn_xor64s = a ^ b;"),
]


def _func_declaration(spec: _FuncSpec) -> str:
    """The Verilog `function ... endfunction` declaration text for
    *spec*, ANSI-style port list (confirmed supported by the parser --
    see `tests/test_model/test_functions_generate.py`'s
    `TestFunctionAnsiStyle`), argument names always `a`, `b`, ... matching
    `spec.body`'s own fixed reference to them.
    """
    ret_sig = "signed " if spec.ret_signed else ""
    arg_names = "abcdefgh"[: len(spec.arg_widths)]
    ports = ", ".join(
        f"input {'signed ' if signed else ''}[{width - 1}:0] {name}"
        for name, width, signed in zip(arg_names, spec.arg_widths, spec.arg_signed, strict=True)
    )
    return f"function {ret_sig}[{spec.ret_width - 1}:0] {spec.name}({ports});\n    begin\n        {spec.body}\n    end\nendfunction"
...
def _gen_expr(
    rng: random.Random,
    depth: int,
    extra_signals: Sequence[tuple[str, int, bool]] = (),
    callables: Sequence[_FuncSpec] = (),
) -> str:
    ...
    *callables* -- user-defined `_FuncSpec`s eligible for the "call" node
    kind (Phase 6, work plan item 3.4), threaded unchanged through every
    recursive call the same way as *extra_signals*. Defaults to `()`, a
    no-op for every pre-existing call site (this file's own `test_
    differential` never passes it, nor does `test_differential_
    statements.py`'s reuse of this function) -- when empty, "call" is
    never added to the kind pool below, so `rng.choice`'s draw sequence
    over the UNCHANGED `_NODE_KINDS` list is identical to before this
    parameter existed. Only `test_differential_functions.py` passes a
    non-empty pool.
    """
    if depth <= 0 or rng.random() < 0.35:
        return _gen_leaf(rng, extra_signals)
    kinds = _NODE_KINDS if not callables else (*_NODE_KINDS, "call")
    kind = rng.choice(kinds)
    if kind == "binary":
        op = rng.choice(_BINARY_OPS)
        lhs = _gen_expr(rng, depth - 1, extra_signals, callables)
        rhs = _gen_expr(rng, depth - 1, extra_signals, callables)
        if op in ("/", "%"):
    ...
    if kind == "unary":
        ...
        return f"({op}{_gen_expr(rng, depth - 1, extra_signals, callables)})"
    if kind == "reduction":
        op = rng.choice(_REDUCTION_OPS)
        return f"({op}{_gen_expr(rng, depth - 1, extra_signals, callables)})"
    if kind == "ternary":
        cond = _gen_expr(rng, depth - 1, extra_signals, callables)
        a = _gen_expr(rng, depth - 1, extra_signals, callables)
        b = _gen_expr(rng, depth - 1, extra_signals, callables)
        return f"({cond} ? {a} : {b})"
    if kind == "concat":
        n = rng.choice((2, 3))
        parts = [_gen_expr(rng, depth - 1, extra_signals, callables) for _ in range(n)]
        return "{" + ", ".join(parts) + "}"
    if kind == "replicate":
        n = rng.choice((2, 3))
        return "{" + str(n) + "{" + _gen_expr(rng, depth - 1, extra_signals, callables) + "}}"
    if kind == "cast":
        cast = rng.choice(("signed", "unsigned"))
        return f"${cast}({_gen_expr(rng, depth - 1, extra_signals, callables)})"
    if kind == "call":
        spec = rng.choice(callables)
        args = ", ".join(_gen_expr(rng, depth - 1, extra_signals, callables) for _ in spec.arg_widths)
